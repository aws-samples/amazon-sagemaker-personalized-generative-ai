...
def main():
    ...
    with smp.model_creation(
        tensor_parallelism=smp.tp_size() > 1, # Note: To enable tensor parallelism
        dtype=torch.bfloat16
        ):
        
        # smp.delay_param_initialization(enabled=True) # TODO # Note: To enable tensor parallelism
    ...
    train_sampler = torch.utils.data.DistributedSampler(
                ...
                shuffle=True,
                seed=args.seed,
                rank=smp.rdp_rank(), # Note: To enable tensor parallelism
                num_replicas=smp.rdp_size(), # Note: To enable tensor parallelism
                drop_last=True,
            )
    ...
    model = smp.DistributedModel(model, 
                                 trace_device="gpu",
                                 backward_passes_per_step=args.gradient_accumulation_steps # Note: To enable tensor parallelism
                                )
    ...
    # Start: This piece of code happend to be before model = smp.DistributedModel...
    ...
    # End: This piece of code happend to be before model = smp.DistributedModel...
